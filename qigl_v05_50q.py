"""
Q-IGL Digital Testbed v0.5 (Scale Test: 50 Qubits)
Focus: Regimes B & C. Comparison: B1 vs K1.
Optimization: Phase-only simulation for GHZ fidelity under dephasing.
"""
import numpy as np
import hashlib
import csv
...
@dataclass
class Config:
    n_qubits: int = 50  # Масштабный тест
    steps: int = 50     # Шаги
    dt: float = 0.05
    trajectories: int = 200 # Больше траекторий
    seed: int = 20260916
...
if __name__ == "__main__":
    ...
    with open("source_hash_v05.txt", "w") as hf:
    ...
    csv_file = "primary_results_v05_50q.csv"
